...
static void RobotSetMotorState(TRobotMotor Motor, TRobotMotorState State)
{
	...
	// Set motor ID
	if (Motor == ROBOT_MOTOR_RIGHT) Command |= 0x10;

	// Set state
	if (State == ROBOT_MOTOR_STATE_FORWARD) Command |= 0x04;
	else if (State == ROBOT_MOTOR_STATE_BACKWARD) Command |= 0x08;
...
{
	unsigned char Command = 0x60; // Opcode

	// Set motor ID
	if (Motor == ROBOT_MOTOR_RIGHT) Command |= 0x10;

	// Set motor direction
	if (Direction == ROBOT_MOTOR_DIRECTION_BACKWARD) Command |= 0x08;

	// Set speed modification
	if (Is_Speed_Increased) Command |= 0x04;
...
unsigned short RobotReadMotorSpeed(TRobotMotor Motor, TRobotMotorDirection Direction)
{
	...
	// Receive value
	Duty_Cycle = (UARTReadByte() << 8) | UARTReadByte();
